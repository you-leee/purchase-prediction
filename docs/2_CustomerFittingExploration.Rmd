---
title: "Customer Fitting Exploration"
...
In this section, we will create some features, that describe customers and test out some models
...
Since we want to predict next years total revenue, we will add it to the features of a customer, so we will only have the 2012 subset of the purchases for training
...
We only want to predict the next years revenue and for that we woudn't need the customers, who didn't spent any money next year (0 amount does not change the result in any way) on the other hand, how can we know, what the model will predict for these customers, if we didn't included them in the training set?
...
### Zero covariates
We can see, that the tenure is a near zero variance predictor
...
We can see, that in all models, total_sales is the most important feature, in the lm model, order_num is the second, but in the gbm model, product_num is. In the mars model, only total_sales got used, all other features were reducted.
...
gbm_RMSE <- gbm_pred$gbm.RMSE
gbm_total_diff <- sum(gbm_pred$gbm.prediction - customer_purchase_test$nexty_total_sales)
gbm_total_diff_perc <- gbm_total_diff * 100 / sum(customer_purchase_test$nexty_total_sales)
...
lm_RMSE <- lm_pred$lm.RMSE
lm_total_diff <- sum(lm_pred$lm.prediction - customer_purchase_test$nexty_total_sales)
lm_total_diff_perc <- lm_total_diff * 100 / sum(customer_purchase_test$nexty_total_sales)
print(paste("RMSE:", lm_RMSE, ", Total difference:", lm_total_diff, ", difference(%):", lm_total_diff_perc))
```
...
mars_RMSE <- mars_pred$earth.RMSE
mars_total_diff <- sum(mars_pred$earth.prediction - customer_purchase_test$nexty_total_sales)
mars_total_diff_perc <- mars_total_diff * 100 / sum(customer_purchase_test$nexty_total_sales)
print(paste("RMSE:", mars_RMSE, ", Total difference:", mars_total_diff, ", difference(%):", mars_total_diff_perc))
```


### Train for whole 2012 dataset and test customer groups
* dec_customers: customers who purchased in december
* onetime_customers: customers with only 1 order
...
Filter customers, who purchased in december and customers, who only ordered once
...
gbm_dec_RMSE <- gbm_pred_dec$gbm.RMSE
gbm_dec_total_diff <- sum(gbm_pred_dec$gbm.prediction - dec_customers$nexty_total_sales)
gbm_dec_total_diff_perc <- gbm_dec_total_diff * 100 / sum(dec_customers$nexty_total_sales)
print(paste("RMSE:", gbm_dec_RMSE, ", Total difference:", gbm_dec_total_diff, ", difference(%):", gbm_dec_total_diff_perc))

# Onetime customers
gbm_pred_onetime <- testNextYearsSales(final_gbm_fit, onetime_customers)
gbm_onetime_RMSE <- gbm_pred_onetime$gbm.RMSE
gbm_onetime_total_diff <- sum(gbm_pred_onetime$gbm.prediction - onetime_customers$nexty_total_sales)
gbm_onetime_total_diff_perc <- gbm_onetime_total_diff * 100 / sum(onetime_customers$nexty_total_sales)
print(paste("RMSE:", gbm_onetime_RMSE, ", Total difference:", gbm_onetime_total_diff, ", difference(%):", gbm_onetime_total_diff_perc))
```
...
lm_dec_RMSE <- lm_pred_dec$lm.RMSE
lm_dec_total_diff <- sum(lm_pred_dec$lm.prediction - dec_customers$nexty_total_sales)
lm_dec_total_diff_perc <- lm_dec_total_diff * 100 / sum(dec_customers$nexty_total_sales)
print(paste("RMSE:", lm_dec_RMSE, ", Total difference:", lm_dec_total_diff, ", difference(%):", lm_dec_total_diff_perc))

# Onetime customers
lm_pred_onetime <- testNextYearsSales(final_lm_fit, onetime_customers)
lm_onetime_RMSE <- lm_pred_onetime$lm.RMSE
lm_onetime_total_diff <- sum(lm_pred_onetime$lm.prediction - onetime_customers$nexty_total_sales)
lm_onetime_total_diff_perc <- lm_onetime_total_diff * 100 / sum(onetime_customers$nexty_total_sales)
print(paste("RMSE:", lm_onetime_RMSE, ", Total difference:", lm_onetime_total_diff, ", difference(%):", lm_onetime_total_diff_perc))
```
...
mars_dec_RMSE <- mars_pred_dec$earth.RMSE
mars_dec_total_diff <- sum(mars_pred_dec$earth.prediction - dec_customers$nexty_total_sales)
mars_dec_total_diff_perc <- mars_dec_total_diff * 100 / sum(dec_customers$nexty_total_sales)
print(paste("RMSE:", mars_dec_RMSE, ", Total difference:", mars_dec_total_diff, ", difference(%):", mars_dec_total_diff_perc))

# Onetime customers
mars_pred_onetime <- testNextYearsSales(final_mars_fit, onetime_customers)
mars_onetime_RMSE <- mars_pred_onetime$earth.RMSE
mars_onetime_total_diff <- sum(mars_pred_onetime$earth.prediction - onetime_customers$nexty_total_sales)
mars_onetime_total_diff_perc <- mars_onetime_total_diff * 100 / sum(onetime_customers$nexty_total_sales)
print(paste("RMSE:", mars_onetime_RMSE, ", Total difference:", mars_onetime_total_diff, ", difference(%):", mars_onetime_total_diff_perc))
```
...
3. Do we need an extra model, separate from customer features?
    + Unfortunatelly forecasting won't work in this case, because seasonability is mainly in yearly data and we only have 2 years of data (2 periods), so can't test it.


## Customer grouping / Clustering with scaled data
Does adding cluster groups help?

Let's log transform first to reduce skewness and plot again the transformed RFMs 
We can see, that the total_sales_amount is appr. gaussian.. so should the nexty_total_sales be, so maybe we can try glm fitting too (later)
...
From the plot, we can see, that it converged around 11 , but there was a break at 2 and 3 clusters too
...
If we fit to 11 groups, we maybe won't have enough data for training each group, so we will try splitting customers into 2 groups only
From the 2 cluster groups, we can see, that:

* There is one group, which has high total_sales, active_days and order_num and a low days_since_last_purchase. In other words it has high monetary, tenure, frequency and high recency, so these are the "best customers"
...
Customer grouping and scaling didn't help much, but we could see, that order_num, product_num and total_sales are appr. linear, and the log transformation helped the skewness of total_sales, so a gaussian linear model with log link could be a good choise

Save the customer RFMs by year and the model and test results for later
```{r}
write.csv(customer_rfm_2012, "data/RFM_2012.csv", row.names = F)
write.csv(customer_rfm_2013, "data/RFM_2013.csv", row.names = F)
```
