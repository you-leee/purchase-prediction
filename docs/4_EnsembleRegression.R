...
# In this ensemble, we will both linear and nonlinear models along with kknn too, hoping to fit the non-linear relationships, what we have seen from earlier and push the model to separate customers, who didn't spent next year from the others
# We will also use some preprocessing, like scaling and removing near zero variance predictors and clean the dataset from too outstanding values
...
### Preprocess data: remove near zero covariates and scale features
print(pp <- preProcess(customers_2012[, -c(1, 7)], 
                       method = c("nzv", "scale"),
                       outcome = customers_2012[, 7]))

# We can see, that active days was discarded
pp$method
...
# Discarde points with too high leverage, influence (in a linear model)
set.seed(32)
...
testData_spline <- cbind(customer_purchase_test, fit = predict(spline_model, customer_purchase_test, type = "response"))
...
# As we can see, kknn was rejected
set.seed(32)
...
# We can see, the the fractional polynomial model did somewhat better, than the others
...
# From the tests we can see, that the ensemble did pretty good both regarding the RMSE and the accuracy in predicting the next years revenue
...
### Test customer spendings for customers who purchased in december and "onetime" customers (order_num == 1)
## Dec customers
...
decData_fitted_spline <- cbind(pp_dec_customers, fit = predict(final_model_spline, pp_dec_customers, type = "response"))
...
# We can see, that so far, this is the best model, so we will choose it for prediction and create an interface for accessing it and predict new data with it
...
kknn_results <- createModelSummary(description = "Knn regression, scaled features, formula = nexty_total_sales ~ order_num + product_num + total_sales + days_since_last_purchase",
